...
if opt.dataset == 'diginetica':
    ...
    n_node = 37484
elif opt.dataset == 'lastfm1_4' or opt.dataset == 'lastfm1_64':
    n_node = 40470
    print(n_node)
else:
    n_node = 310
...
print(opt)
best_result = [0, 0]
best_result_train = [0, 0]
best_epoch = [0, 0]
for epoch in range(opt.epoch):
    print('epoch: ', epoch, '===========================================')
    slices = train_data.generate_batch(model.batch_size)
    fetches = [model.opt, model.loss_train, model.global_step, model.score_train]
    print('start training: ', datetime.datetime.now())
    hit_train, mrr_train, loss_ = [], [], []
    for i, j in tqdm(zip(slices, np.arange(len(slices)))):
        adj_in, adj_out, alias, item, mask, targets = train_data.get_slice(i)
        _ , loss, _, scores = model.run(fetches, targets, item, adj_in, adj_out, alias,  mask)
        loss_.append(loss)
        index = np.argsort(scores, 1)[:, -20:]
        for score, target in zip(index, targets):
            hit_train.append(np.isin(target - 1, score))
            if len(np.where(score == target - 1)[0]) == 0:
                mrr_train.append(0)
            else:
                mrr_train.append(1 / (20-np.where(score == target - 1)[0][0]))
    loss = np.mean(loss_)
    ...
    hit_train = np.mean(hit_train)*100
    mrr_train = np.mean(mrr_train)*100
    if hit_train >= best_result[0]:
        best_result_train[0] = hit_train
        # best_epoch[0] = epoch
    if mrr >= best_result[1]:
        best_result_train[1] = mrr_train
        # best_epoch[1]=epoch
    print('train_loss:\t%.4f\tRecall@20:\t%.4f\tMMR@20:\t%.4f\ttest_loss:\t%4f\tRecall@20:\t%.4f\tMMR@20:\t%.4f\tEpoch:\t%d,\t%d'%
          (loss, best_result_train[0], best_result_train[1], test_loss, best_result[0], best_result[1], best_epoch[0], best_epoch[1]))
